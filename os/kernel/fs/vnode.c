        ...
        for (link = list->l_next; link != list; link = link->l_next) {
                ...
                if (((vn->vn_fs->fs_root == vn) && (1 < refs))
                    || ((vn->vn_fs->fs_root != vn) && refs)) {
                        dbg(DBG_ALL, "Vnode %ld addr %p mode %x device %x flags %x is still in use with refcount=%d and %d res pages\n",
                            (long)vn->vn_vno, vn, vn->vn_mode, vn->vn_devid, vn->vn_flags, vn->vn_refcount, vn->vn_nrespages);
//                        dbg(DBG_ALL, "Vn 0x%p Vnode %ld mode %x device %x flags %x is still in use with refcount=%d and %d res pages\n",
//                            vn, (long)vn->vn_vno, vn->vn_mode, vn->vn_devid, vn->vn_flags, vn->vn_refcount, vn->vn_nrespages);
...
//        NOT_YET_IMPLEMENTED("VFS: special_file_read");
	...
	if (S_ISCHR(file->vn_mode)) { /* for a character device, all these pointers must be non-NULL */
		KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
      dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
		ret = file->vn_cdev->cd_ops->read( file->vn_cdev, offset, buf, count); 
	}
	//KASSERT(ret >= 0);
...
//        NOT_YET_IMPLEMENTED("VFS: special_file_write");
	...
	if (S_ISCHR(file->vn_mode)) { /* for a character device, all these pointers must be non-NULL */
	   KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
		dbg(DBG_PRINT, "(GRADING2A 1.b)\n");
		ret = file->vn_cdev->cd_ops->write( file->vn_cdev, offset, buf, count);
	}
	//KASSERT(ret >= 0);
	dbg(DBG_PRINT, "(GRADING2A)\n");
	return ret;
...
special_file_fillpage(vnode_t *file, off_t offset, void *pagebuf)
{
        ...
        return 0;
//not reached//        KASSERT(NULL != file);
//not reached//        KASSERT(NULL != pagebuf);
//not reached//		  dbg(DBG_TEST, "(NEWPATH)\n");
//not reached//
//not reached//        return file->vn_cdev->cd_ops->fillpage(file, offset, pagebuf);
}
...
//not reached//		  dbg(DBG_TEST, "(NEWPATH)\n");
//not reached//
//not reached//        return file->vn_cdev->cd_ops->dirtypage(file, offset);
}
