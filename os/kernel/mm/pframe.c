...
pframe_t * pf=NULL;
        ...
        while (1){
            pget = pframe_get_resident(o, pagenum);
				dbg(DBG_PRINT, "(GRADING3A)\n");
            if (pget != NULL){ //the page is found
					 dbg(DBG_PRINT, "(GRADING3A)\n");
                if (pframe_is_busy(pget)){// the page is busy
                    //while(pframe_is_busy(pget)){ //wait
                    //}
							sched_sleep_on(&pget->pf_waitq);
					dbg(DBG_PRINT, "(GRADING3A)\n");		
                    continue; //try again
                }
                else{// found the page and not busy
                    *result = pget;
            ...
            else{// the page is not found
					 KASSERT(!pageoutd_needed()); // not reached in k3
					dbg(DBG_PRINT, "(GRADING3A)\n");		
					 //if (pageoutd_needed()){
					 //   pageoutd_wakeup();
...
pframe_pin(pframe_t *pf)
{
	...
	/*
	if pin count == 0{ // not pinned
		need to remove from allocated list
		add to pinned list // order doesn't matter
	decrement nallocated 
	incr npinned
	}
	*/
	if (!pframe_is_pinned(pf)){
		list_remove(&pf->pf_link); // remove from allocated list
...
//        NOT_YET_IMPLEMENTED("VM: pframe_unpin");
	...
	if(!pframe_is_pinned(pf)){
		npinned--;
		nallocated++;
		list_remove(&pf->pf_link); // remove from pinned list
		list_insert_tail(&alloc_list, &pf->pf_link); // 
		dbg(DBG_PRINT, "(GRADING3A)\n");
	}
	dbg(DBG_PRINT, "(GRADING3A)\n");
