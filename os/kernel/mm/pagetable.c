...
pt_map(pagedir_t *pd, uintptr_t vaddr, uintptr_t paddr, uint32_t pdflags, uint32_t ptflags)
{
        ...
        if (!(PT_PRESENT & pd->pd_physical[index])) {
                if (NULL == (pt = page_alloc())) { // allocate a new page as the page table for that particular PDE
                        return -ENOMEM;
                } else {
                        KASSERT((pdflags & ~PAGE_MASK) == pdflags);
                        memset(pt, 0, PAGE_SIZE);
                        pd->pd_physical[index] = pt_virt_to_phys((uintptr_t)pt) | pdflags; // comment: PDE stores physical address of page tables + flags
                        pd->pd_virtual[index] = pt; // comment: pt will have virtual address
                }
        } else {
        ...
        pt[index] = paddr | ptflags; // we have already allocate the physical page, so just assign ppn with flag as PTE into the page table
