...
kmutex_lock(kmutex_t *mtx)
{
        //NOT_YET_IMPLEMENTED("PROCS: kmutex_lock");
		  ...
		  //else{
		  // TODO Tricky! need to review: when kmutex_return curthr must be holding the lock (see slide 39 05_processor.pdf)	  
...
kmutex_lock_cancellable(kmutex_t *mtx)
{
        //NOT_YET_IMPLEMENTED("PROCS: kmutex_lock_cancellable");
		  ...
		  // 	kthread_exit((void *) -ECANCELED);
		  // 	// TODO check: how to return?
		  // 	return -EINTR;
		  //}
		  // not cancelled
/* please use TWO consecutive "conforming dbg() calls" for this since this function is not called if you just start and stop weenix */
        KASSERT(curthr && (curthr != mtx->km_holder)); /* curthr must be valid and it must not be holding the mutex (mtx) already */
		  ...
		  if (mtx->km_holder ){ // can they call kmutex_lock, then kmutex_lock_cancellable?
			  if (sched_cancellable_sleep_on(&(mtx->km_waitq)) == -EINTR){
				   if (curthr == mtx->km_holder){ // added
						kmutex_unlock(mtx);
