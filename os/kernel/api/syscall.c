...
//	if (copy_from_user(&kern_args, arg, sizeof(kern_args)) < 0) {
	KASSERT (copy_from_user(&kern_args, arg, sizeof(kern_args)) >= 0);
...
//    }
    //path = user_strdup(kern_args.path);
	 size_t pi;
	 for (pi = 0; pi < ((uint32_t)PAGE_ALIGN_UP(kern_args.nbytes) / PAGE_SIZE); pi++){
	    temp_buf = page_alloc();
	    
	    // system out of memory
		 KASSERT(temp_buf != NULL);
		dbg(DBG_PRINT, "(GRADING3A)\n");
//	    if (temp_buf == NULL) {
//	    	curthr->kt_errno = EFAULT;
//            dbg(DBG_PRINT, "(NEWPATH)\n");
...
//	    }
	    // int do_read(int fd, void *buf, size_t nbytes)
		 KASSERT(((int)kern_args.nbytes - (int)(pi*PAGE_SIZE)) >= 0);
        dbg(DBG_PRINT, "(GRADING3A)\n");
	   	ret = do_read(kern_args.fd, temp_buf, (kern_args.nbytes - pi*PAGE_SIZE));
	   	if (ret >= 0) {
				KASSERT(copy_to_user((void*)((uint32_t)kern_args.buf + pi*PAGE_SIZE), temp_buf, ret) >= 0);
				dbg(DBG_PRINT, "(GRADING3A)\n");
	    		//if ( copy_to_user((void*)((uint32_t)kern_args.buf + pi*PAGE_SIZE), temp_buf, ret) < 0){
				//	curthr->kt_errno = EFAULT;
            //    dbg(DBG_PRINT, "(NEWPATH)\n");
	        	//	return -1;
				//}
...
//        return -1;
   ...
   // }
    ...
    for (pi = 0; pi < ((uint32_t)PAGE_ALIGN_UP(kern_args.nbytes) / PAGE_SIZE); pi++){
        temp_buf = page_alloc();
        size_t len = 0;
        if ((pi+1)*PAGE_SIZE >kern_args.nbytes ){
            len = kern_args.nbytes - pi*PAGE_SIZE;
        		dbg(DBG_PRINT, "(GRADING3A)\n");
        }else{
            len = PAGE_SIZE;
        ...
        KASSERT(((int)kern_args.nbytes - (int)(pi*PAGE_SIZE)) >= 0);
			KASSERT(copy_from_user(temp_buf,(void*)((uint32_t)kern_args.buf + pi*PAGE_SIZE), len ) >=0);
        	dbg(DBG_PRINT, "(GRADING3A)\n");
        //if ( copy_from_user(temp_buf,(void*)((uint32_t)kern_args.buf + pi*PAGE_SIZE), len ) < 0){
        //    curthr->kt_errno = EFAULT;
        //		dbg(DBG_PRINT, "(NEWPATH)\n");
        //    return -1;
        //}else{

            if ( (ret = do_write(kern_args.fd, temp_buf, len)) < 0){
                curthr->kt_errno = -ret;
        			 dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -1;       
            }else{
     ...
     return total_ret;
//   	if (ret >= 0) {
//    		ret = copy_to_user(kern_args.buf, temp_buf, sizeof(ret));
//    	}
//    page_free(temp_buf);
//    return ret;
}
...
//        return -1;
	...
	dirent_t temp;
	// count: the number bytes in buffer
	int count;
	// ret: return val of do_get, it is either 0 or sizeof(dirent_t), or -errno.
	int ret;
	int total_ret = 0;
	KASSERT(copy_from_user(&kern_args, arg, sizeof(kern_args)) >= 0);
   dbg(DBG_PRINT, "(GRADING3A)\n");
   ...
   // }
    ...
    count = (int) kern_args.count;


	for (int i=0; i < (count/(int)sizeof(dirent_t)); i++) {
		// call do_getdent in a loop
		// do_getdent(int fd, struct dirent *dirp)
		ret = do_getdent(kern_args.fd, &temp);
		//if (getdent_args_t->count)

		// ret = do_read(kern_args.fd, temp+(sizedof(diren_t))*i, xxx );

		// or an error occurs
//		if (ret == -EBADF || ret == -ENOTDIR) {
		if (ret < 0){
			curthr->kt_errno = -ret;
            dbg(DBG_PRINT, "(GRADING3D 1)\n");
        	return -1;
		...
		if (ret > 0) {
			KASSERT( copy_to_user(kern_args.dirp + i, &temp, ret) >= 0);
         dbg(DBG_PRINT, "(GRADING3A)\n");
    		//if ( copy_to_user(kern_args.dirp + i, &temp, ret) < 0) {
			//	curthr->kt_errno = EFAULT;
         //       dbg(DBG_PRINT, "(NEWPATH)\n");
			//	return -1;
			//}
